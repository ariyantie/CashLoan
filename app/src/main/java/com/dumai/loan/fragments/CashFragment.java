...
import com.dumai.loan.activity.ScheduledExActivity;
import com.dumai.loan.adapter.GalleryAdapter;
import com.dumai.loan.base.BaseWGalleryActivity;
import com.dumai.loan.view.PerfectDataDialog;
...
public class CashFragment extends BaseWGalleryActivity {
    ...
    @BindView(R.id.ll_ex_repayment)
    LinearLayout llExRepayment;
    @BindView(R.id.ll_ex_schedule)
    LinearLayout llExSchedule;
    @BindView(R.id.ll_ex_rate)
    LinearLayout llExRate;
    ...
    @OnClick({R.id.ll_ex_schedule, R.id.ll_sing_loan, R.id.ll_cash_staging})
    public void onViewClicked(View view) {
        View dia_view = LayoutInflater.from(getContext()).inflate(R.layout.dialog_layout, null, false);
        switch (view.getId()) {
            case R.id.ll_ex_schedule:
                startActivity(new Intent(getContext(), ScheduledExActivity.class));
                break;
